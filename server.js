...
app.use(express.static(path.join(__dirname, 'dist/homeworkv8')));
...
app.get("/weatherCard", (req, res) => {
  ...
  app.get("/getCityImages", (req, res) => {
    const API_key = <YOUR_GOOGLE_CUSTOM_API_KEY>;
    const SEARCH_ENGINE_ID = <YOUR_SEARCH_ENGINE_ID>;
    const url = "https://www.googleapis.com/customsearch/v1?q=" + req.query.city + "%20City&cx=" + SEARCH_ENGINE_ID + "&imgSize=huge&imgType=photo&num=8&searchType=image&key=" + API_key;
    request(url, function (error, response, body) {
        body = JSON.parse(body);
        res.send(body);
    });
